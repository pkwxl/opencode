...
- 稳定性(2026-09-08 修订):评估是**轮首一次性决策**——结论随本轮标记固化
  (.auto/tool.json 的 phases 键,轮首随标记建立写入),续跑直接复用固化值,
  不再重开提取会话、不再重新评估。旧机制轮次(标记尚无 phases 键)回落已落盘
  文档的记录值(prior-kb 幂等守卫对"轮已推进"的续跑回落接受无前缀存量,见核心
  knowledge.ts existingPriorKnowledge);固化值/记录值给出的流程不含台账已完成
  阶段时钳制回完整流程(`resumePhases`,裁剪不得低于已完成进度)。提取失败 →
  无 verdict → 完整流程;dryrun 不做前置会话 → 维持默认。
- 人工兜底:手工把 prior 文档协议行改为 full(或删除该节)即恢复完整流程;
  已固化轮次需同步编辑 .auto/tool.json 的 phases 键(或删除该文件重开一轮)。
