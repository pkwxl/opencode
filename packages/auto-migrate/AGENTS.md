...
`@opencode-ai/auto-migrate` 是简易 CLI 外壳(bin `auto-migrate`):无子命令,`src/index.ts` 做参数解析与配置固化/冲突校验后委托 `src/tool.ts` 主编排(轮首建立 docs/R-NN → 前置知识提取 → 参数推断 → 二次迁移,自动推进至结束,中断续跑;流程缺省完整 admtvk,--phases 显式裁剪——设计见 docs/specialized-tool-design.md §10);前一轮彻底完成后 `--next-path <相对路径>` 一条命令开启新一轮(设计见 docs/specialized-tool-design.md §9)。全部机制实现在核心库 `@opencode-ai/auto-core`(workspace 依赖,子路径导入)。注释与用户可见文案使用中文。
