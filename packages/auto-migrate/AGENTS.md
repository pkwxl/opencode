...
`@opencode-ai/auto-migrate` 是简易 CLI 外壳(bin `auto-migrate`):无子命令,`src/index.ts` 做参数解析与配置固化/冲突校验后委托 `src/tool.ts` 主编排(轮首建立 docs/R-NN → 前置知识提取 → 参数推断 → 二次迁移,自动推进至结束,中断续跑;流程缺省完整 admtvk,--phases 显式裁剪——设计见 docs/specialized-tool-design.md §10);前一轮彻底完成后 `--next-path <相对路径>` 一条命令开启新一轮(设计见 docs/specialized-tool-design.md §9)。本轮标记 `.auto/tool.json` 缺失/损坏时自动恢复(driver 推导锚点 + AI 恢复会话重建 + driver 校验,设计见 docs/specialized-tool-design.md §1)。全部机制实现在核心库 `@opencode-ai/auto-core`(workspace 依赖,子路径导入)。注释与用户可见文案使用中文。
...
- 本包只含简易壳(`src/index.ts`、`src/tool.ts`)、壳层提示词模板(`src/templates/`,本轮标记恢复)、构建脚本与测试;核心 src、模板与设计文档在 `../auto-core`。
...
- **模板必须保持 `with { type: "file" }` 导入**(核心模板经 `@opencode-ai/auto-core/templates/*` 子路径),这是编译时嵌入二进制的唯一方式;infer-source/prior-knowledge 等模板已内置核心,壳层私有模板(本轮标记恢复 `src/templates/tool-state-recovery.md`)经 `registerTemplate` 在 `src/tool.ts` 顶层注册。
