...
`@opencode-ai/auto-migrate` 是简易 CLI 外壳(bin `auto-migrate`):无子命令,`src/index.ts` 做参数解析与配置固化/冲突校验后委托 `src/tool.ts` 主编排(前置知识提取 → 现场清理 → 参数推断 → 完整 admtvk 二次迁移,自动推进至结束,中断续跑)。全部机制实现在核心库 `@opencode-ai/auto-core`(workspace 依赖,子路径导入)。注释与用户可见文案使用中文。
