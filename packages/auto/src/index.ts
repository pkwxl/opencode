...
import { CONFIG_DEFAULTS, legacyModeFallback, loadProjectConfig, mergeProjectConfig, saveProjectConfig, type ProjectConfig } from "./config"
import { setInteractive, setLogFile, setVerbose } from "./log"
...
if (["init", "continue", "run", "check", "status"].includes(args[0] ?? "")) {
  console.error(`子命令 ${args[0]} 已移除: 本工具无子命令,直接运行 opencode-auto [dir] 即可(断点恢复与续轮归档自动处理)`)
  process.exit(1)
}
...
if (positional.length > 1) {
  console.error(`用法: opencode-auto [dir] [选项](只接受一个目录参数,当前: ${positional.join(" ")})`)
  process.exit(1)
}
