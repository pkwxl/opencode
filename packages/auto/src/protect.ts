...
// opencode.json and AGENTS.md are chmod'd 0o444 so agent sessions cannot
// modify them by mistake (defense in depth on top of the prompt contract —
// a same-user process could still chmod them back via bash, so this is a
// guardrail, not a security boundary). Driver writes call allowWrite /
// reprotect around each mutation; runAll restores writability in a finally
// block so a human can edit the files after the driver stops.
const FILES = ["PLAN.md", "CURRENT.md", "opencode.json", "AGENTS.md"]
