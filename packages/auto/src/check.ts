...
// 违背特征: 执行动词 + 验证语义(中文/英文),或会话执行 git 提交。动词与验证词的
// 距离收得很紧,避免"执行会话链…验收""执行权原则…验证原则块"这类同句误报;
// 排除"可执行(文件)"中的执行与 runVerifyScript 这类标识符内的 verify。
const VIOLATION_PATTERNS: RegExp[] = [
  /(?<!可)(运行|执行|跑)[^。\n]{0,8}(验证|验收|(?<!\w)verify)/i,
  /\b(run|execute|perform)\b[^.\n]{0,40}\b(verify|verification|acceptance)\b/i,
  // 提交: 会话内跑 git add/commit 或"提交全部/所有改动"类指令(统一提交由 driver
  // 在会话后执行);"提交信息/提交 SHA"等名词性表述不匹配。
  /\bgit\s+(add|commit)\b/i,
  /提交(全部|所有|一次)?(未提交)?(改动|变更|代码)/,
]
...
// 一行是否与原则相违背: 命中"执行动词 + 验证语义"或"会话执行 git 提交",且不是
// 否定句、不归属 driver、不是 PLAN.md 字段行。
