...
结论)。verify 的处理权在 driver:随后由它启动任务级审核会话(旁路独立会话,总是
新建,不进会话链)验收整个任务:审核者可读代码、自行运行或调整检查命令(任务
`verify` 字段中声明的命令仅为参考),判定写入 `.auto/verify.md`(末行
`结论: 通过|差距`),driver 解析结论;通过后 driver 标 `[done]`(审核实际运行的
命令记入 `verified` 字段);差距由 driver 反馈回执行会话续跑修复,之后重新收尾与
验收,最多 3 轮,耗尽阻塞(off 模式除外,见上)。
