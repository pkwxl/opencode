...
4. **前置知识提取**(见[阶段化流程](#阶段化流程固定-admtvk)的"前置知识提取"):
   在旧有迁移现场原状上通读 brief.md、docs/ 全树与 git log 概览,蒸馏为
   `docs/prior-kb/prior-<时间戳>.md`;已有非空产物则跳过,失败仅警告后继续;
5. **现场清理**(原 continue 归档):本轮标记 `.auto/tool.json` 不存在时,目录里的
   阶段状态一律视为"别人的"遗留(本工具此前轮次删除标记后、或人工/其他工具的迁
   移结果)——阶段台账记录过完成阶段或无法解析、`PLAN.md` 有任务或
   `docs/migration-kb/` 有残留,即把现场归档到 `docs/phases/round-<N>/` 并重置
   `PLAN.md` 为空模板,本轮从头规划;随后建立本轮标记 `{ "round": N }`——此后创
   建的文件视为"自己的",中断重跑依标记续跑、不再清理(标记已存在时本步整体跳过);
...
8. 全部阶段完成(退出码 0)→ 完成标记 `.auto/tool.json` 写入 `{ "done": true }`。
...
| `--verbose [true]` | 终端同步显示会话内全部消息部件(文本、工具调用、推理、步骤等)与上下文用量/占比,每行带时间戳,并每 10 秒列出 git status 新出现的变动文件(含子目录中的嵌套 git 仓库);这些明细始终写入日志文件,该选项只决定是否上终端 |
| `--interactive` / `-i` | 旁路交互(与 `--verbose` 互斥):终端保持非 verbose 的干净输出并常驻等待人工输入,回车把输入作为额外用户消息发往当前活动会话(steer 语义,在下一 provider turn 边界处理;无活动会话时输入丢弃并提示),等待输入不阻塞正常执行。`--wait-answer`/`--wait-between` 的人工等待也经这条输入行接收,ask 结束后恢复接收会话消息 |
...
它是与选项无关的完整审计记录:终端的全部输出与 verbose 明细(会话部件、
上下文用量)都逐条带时间戳直写该文件(进程中断也不丢已输出内容);
`--verbose` 的变动文件列表随其终端行为记录,仅在启用时写入。
...
`continue` 子命令已移除——续轮由主程序**自动处理**:启动时若本轮标记
`.auto/tool.json` 不存在,目录里的阶段状态一律视为"别人的"遗留(本工具此前轮次
删除标记后、或人工/其他工具的迁移结果)。主程序先做前置知识提取(在旧有迁移现场
原状上分析),再把遗留现场归档到 `docs/phases/round-<N>/` 并建立本轮标记,从头
规划新一轮继续迁移——目标是**让迁移结果与源系统更加完整、一致**(补齐遗漏、对齐
残余差距),而不是重做已完成的工作:

```sh
opencode-auto <dir> --source-dir legacy --source-path pkg --dest-dir app   # 第 1 轮: a→d→m→t→v→k 全部完成,写 .auto/tool.json {done:true}
rm <dir>/.auto/tool.json    # 显式开启新一轮(或: 目录里本就有迁移遗留,首次运行会自动归档它)
opencode-auto <dir> -p "第二轮聚焦补齐 API 覆盖差距"                        # 第 2 轮: 前置知识提取 → 现场清理 → 推断/规划 → 完整 admtvk
```

- **清理判定**:标记不存在,且阶段台账记录过完成阶段(或无法解析)、`PLAN.md`
  有任务、`docs/migration-kb/` 有残留之一 → 归档清理;标记已建立 = 本轮在跑,
  中断重跑依断点续跑,**绝不清理自己的现场**。运行中删除标记 = 显式开启新一轮
  (进行中的轮次会被归档)。
...
- **本轮标记 `.auto/tool.json`**(非版本化):现场清理后写入 `{ "round": N }`
  (本轮开始,此后创建的文件视为"自己的");全部阶段完成(退出码 0)时写入
