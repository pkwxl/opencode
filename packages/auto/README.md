...
- **阶段循环(`run`)**:配置 `phases ≠ "m"` 时 `run` 按"规划 → 执行 → 交接"推进到
  ...
  - **k(知识提炼)阶段例外**:k 阶段不开规划会话、不向 `PLAN.md` 填任务——
    ...
    设计,见 `docs/fixme-knowledge-design.md` 文首修订节),通读阶段台账
    (`docs/phases.md`)与各阶段交接文档(`docs/handovers/R<N>-<字母>-<名称>.md`
    优先),把最终验证过的迁移经验蒸馏为
    `docs/migration-kb/R<N>-migration-<时间戳>.md`(永久路径,不随轮次归档移动;
    章节骨架:迁移概要/API 与类型映射/实现模式/坑点与边界情况/可复用规则/设计
    偏差与重要决策/验证证据/参考)。
    **提取失败不污染退出码**:会话受阻或两次未产出仅打 ⚠ 警告,k 阶段照常交接
    (迁移成功不被文档生成失败反向污染;重试 = 删台账 k 行与 `docs/migration-kb/`
    内本轮 `R<N>-` 前缀文档后重跑);交接前中断则重跑时按本轮前缀幂等跳过已产出
    的文档(前几轮的知识文档不算本轮已提取)。知识文档随统一提交入库。人工在
    k 阶段自行向 `PLAN.md` 填任务时走通用执行/交接路由,提取挂点不触发。
  - **交接**:本阶段任务全 done 后,先开**交接蒸馏会话**(旁路一次性,通读本阶段
    `PLAN.md` 与 `docs/` 产物,提炼出 `docs/handovers/R<N>-<字母>-<名称>.md`
    永久路径下的交接文档,必备四个小节:关键决策 / 约束与坑 / 下一阶段必读清单 /
    产物索引;产物缺失带反馈重试一次,仍失败按隐性阻塞退出码 2),再由 driver
    机械归档——`PLAN.md` 拷贝进归档目录 `docs/phases/<字母>-<英文名>/`(仅收
    过期状态文件)后、根目录的重置为空模板,台账追加一行,整体作为一次统一提交
...
- **归档与重置**:上一轮整体归档到 `docs/phases/round-<N>/`(N = 被归档轮次)——
  台账(`phases.md`)、各阶段归档目录、轮末根 `PLAN.md` 快照与根 `AGENTS.md`
  快照(拷贝,原文件保留)一并移入;台账随之消失 = 空台账、根 `PLAN.md` 由模板
  循环重建为空模板。交接文档(`docs/handovers/`)与知识文档
  (`docs/migration-kb/`、`docs/prior-kb/`)为永久路径,不随归档移动;新一轮
  k 阶段按 `R<N>-` 前缀守卫重新提取,不与上一轮文档混淆。各步为 rename 且台账
  最后移动,归档中断后重跑 `continue` 自然续完。
- **结论注入**:新一轮**首个阶段规划会话**注入上一轮结论摘录——各阶段归档目录
  索引 + 最终完成阶段交接文档全文(`docs/handovers/R<N>-….md`)+ 迁移知识文档
  全文(`docs/migration-kb/` 的 `R<N>-` 前缀文件;蒸馏产物仍是唯一通道,原始
  产物按索引可达,归档就在工作目录内);后续阶段照常走本轮 handover 蒸馏链,
  不重复注入。
