...
非法或记录了 `phases` 之外的阶段字母);`2` 阻塞或未完成为 pending,等待人工介入(含阶段规划会话受阻与终审闭环熔断);`3` `--interactive` 下收到 `/exit`、已在安全边界处暂停退出(不需要人工介入,重新运行即可完整恢复);`130` 被强制终止。
...
公共部分(**收尾与验收**):一个会话统一更新 docs/、`docs/T-NNN/report.md`
...
3. **AI 判定**:旁路独立判定会话(总是新建,不进会话链)直读输出文件
   ...
   产物文档(`docs/T-NNN/**`)做**引用门禁**确定性预扫(实验开关
   `OPENCODE_AUTO_REF_CHECK=on` 时启用,缺省 off 不做)——失效引用(路径不存在、
...
审核升级为对整个计划的设计、实现与文档的**最终全面审核**。审计报告统一写
`docs/T-NNN/audit.md`(终审与普通审核同路径),结论写 `.auto/review.md`
...
写入测试交接文档后结束会话(文档按执行范围命名: 子任务为
`docs/<任务ID>/S<两位序号>/testhandoff.md`,整任务会话与验收修复轮为
`docs/<任务ID>/testhandoff.md`,交接只对本执行范围生效——下一子任务不会误读
上一子任务的遗留交接;文档缺失带反馈重试一次,仍缺失按
隐性阻塞停机);子任务完成时该子任务的测试交接文档随之清除;driver 随即开新会话(上下文已超限,会话复用规则自动新建),以续跑
...
- **轮次专用目录 `docs/R-NN/`**:阶段化流程(`phases ≠ "m"`)的每一轮是一个自
  ...
  轮内 `PLAN.md`(本轮任务台账,根部 `PLAN.md` 是指向它的相对符号链接——单一
  事实源、零漂移,会话与 driver 的 "PLAN.md" 路径认知不变,写经链接落轮内;
  符号链接创建失败的环境兜底为副本并日志说明)、阶段台账 `phases.md`、阶段归档
  目录 `<字母>-<英文名>/`、交接文档 `handovers/<字母>-<英文名>.md`、阶段级自由
  产物 `phase-docs/<字母>-<英文名>/`、轮首 AGENTS.md 快照 `AGENTS.md.bak`
  (`.bak` 后缀避免被当指令自动加载)、知识文档 `migration-kb.md`/`prior-kb.md`。
  旧布局存量(根 `docs/phases.md` 台账、`docs/phases/` 平铺与 `round-<N>/`
  归档、`docs/handovers/`、`docs/migration-kb/`、`docs/prior-kb/` 平铺)原地
  保留为**只读回落**源,写只写新布局;旧布局在途轮次不被打断,下次 continue
  起进入新布局,混合项目(旧 `round-1..4` + 新 `R-05`)自然续号。
- **阶段台账(轮内 `docs/R-NN/phases.md`)**:阶段状态是**推导式**的——台账
  (版本化、人工可编辑)记录已完成阶段,当前阶段 = `phases` 串中第一个未在台账
  出现的字母:
...
- **人工回退**:回退到某阶段 = ① 从本轮台账删除该阶段及其后的全部行;② 删除
  轮内对应 `<字母>-*/` 归档目录(或把其中 PLAN.md 拷回轮内根 `PLAN.md` 续跑);
  ③ 重跑 `run`。推导式状态使回退无需专门代码支持。
...
- **阶段循环(`run`)**:配置 `phases ≠ "m"` 时 `run` 按"规划 → 执行 → 交接"推进到
  ...
  - **k(知识提炼)阶段例外**:k 阶段不开规划会话、不向 `PLAN.md` 填任务——
    ...
    **提取失败不污染退出码**:会话受阻或两次未产出仅打 ⚠ 警告,k 阶段照常交接
    (迁移成功不被文档生成失败反向污染;重试 = 删台账 k 行与轮内
    `migration-kb.md` 后重跑);交接前中断则重跑时按轮内文档幂等跳过已产出
    (前几轮的知识文档不算本轮已提取)。知识文档随统一提交入库。人工在
    k 阶段自行向 `PLAN.md` 填任务时走通用执行/交接路由,提取挂点不触发。
  - **交接**:本阶段任务全 done 后,先开**交接蒸馏会话**(旁路一次性,通读本阶段
    `PLAN.md` 与 `docs/` 产物,提炼出轮内 `docs/R-NN/handovers/<字母>-<英文名>.md`
    交接文档,必备四个小节:关键决策 / 约束与坑 / 下一阶段必读清单 /
    产物索引;产物缺失带反馈重试一次,仍失败按隐性阻塞退出码 2),再由 driver
    机械归档——`PLAN.md` 快照拷贝进轮内阶段归档目录
    `docs/R-NN/<字母>-<英文名>/`(仅收过期状态文件)后、轮内 `PLAN.md` 经根
    符号链接重置为空模板,台账(轮内 `phases.md`)追加一行,整体作为一次统一
    提交(`Auto-Stage: phase-transition`)落账。本阶段的 `docs/` 产物文档
    (`docs/T-*/` 等)为永久路径,交接不搬移。各步幂等,交接中途断电/Ctrl+C 后
    重跑会自行补完(含补写台账)。
  - 台账覆盖 `phases` 全部字母 → 退出码 0(`✓ 全部阶段已完成`)。
- **阶段进度行**:`run` 启动横幅与 `status` 在配置摘要后打印一行进度,`✓` = 台账
  已记录、`▶` = 当前阶段、其余字母 = 未开始;续轮(轮次目录 `docs/R-NN` 最大号
  \> 1)时带轮次标注:
...
- **轮首建立(不再归档搬移)**:建立新一轮轮次目录 `docs/R-(N+1)/`——轮内
  `PLAN.md` 为空模板(新轮目录恒空,新一轮从头规划,无现场可清)、根 `PLAN.md`
  符号链接重指轮内、根 `AGENTS.md` 快照写入轮内 `AGENTS.md.bak`(轮首一次性,
  已存在不重写)。上一轮轮次目录 `docs/R-N/`(台账、各阶段归档、轮末 PLAN、
  交接与知识文档)原样保留——落盘即永久,绝不搬移/删除。各步幂等,中断后重跑
  `continue` 自然续完(轮目录已建 = 既有内容不重写,链接重建不漂移)。
- **结论注入**:新一轮**首个阶段规划会话**注入上一轮结论摘录——上一轮轮内
  阶段归档目录索引 + 最终完成阶段交接文档全文(`docs/R-N/handovers/`)+ 迁移
  知识文档全文(`docs/R-N/migration-kb.md`;蒸馏产物仍是唯一通道,原始产物按
  索引可达,轮目录就在工作目录内);后续阶段照常走本轮 handover 蒸馏链,
  不重复注入。
- **参数修订**:`--phases` 可为任何合法值(新轮目录台账恒空,不受前缀护栏约束,
...
1. **生成会话**:旁路一次性会话把下一阶段规划成自包含任务提案
   `docs/T-F<k>/plan-<阶段>-r<轮>.md`(锚定即将追加的 `T-F<k>` 目录,只规划不实施);
2. **追加真任务**:driver 解析提案追加为 `T-F<k>` 任务(不写 `verify` 字段——
   终审任务强制跳过任务级验收);
3. **路由**:任务完成且带 `final` 标记后,driver 解析阶段报告末行协议确定性路由
   (各阶段报告锚定产出它的任务自己的 `docs/T-F<k>/` 目录):
   - **终审审计**(报告 `docs/T-F<k>/audit-r<N>.md`,末两行 `结论: <概述>` 与
     `策略: 重构|修补|无`):策略"无"直达终审收尾(跳过修复与回归验证,原任务
     已有任务级 verify 兜底);否则生成同轮修复任务;
   - **修复**(报告名按策略取 `docs/T-F<k>/refactor-r<N>.md` 或
     `docs/T-F<k>/patch-r<N>.md`,自由正文):完成后生成同轮回归验证任务;
   - **回归验证**(报告 `docs/T-F<k>/validate-r<N>.md`,末行
     `结论: 通过` 或 `结论: 差距 <描述>`):通过生成终审收尾任务;差距回退
     audit@<N+1>(提示词要求聚焦残余差距、不做全量重审),审计轮耗尽则**熔断**——
     阻塞最后的终审任务,残余差距与报告指针写入 `question` 字段,退出码 2;
   - **终审收尾**(报告 `docs/T-F<k>/finalize.md`,自由正文):完成后终审结束;
...
- **中断恢复零新增状态**:重新运行时由(带 `final` 标记的任务及其状态,
  `docs/T-F<k>/` 产物)重新路由——下一阶段任务已存在则主循环直接拾取;提案已产出
...
修复(rename 改写)与 verify 门禁见「统一提交」与「验收判定」相关章节。
