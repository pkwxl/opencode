...
(看门狗取配置的 `idleTime` / `idleMax`);`--dryrun` 读配置的 `agent` /
...
`--review` 的质量审核改为此时串行执行,driver 托管脚本看门狗也不参与。
...
(分解 / 整任务执行 / 逐子任务 / 收尾 / verify{脚本生成/执行/判定/修复轮,含修复轮
与重验轮计数、判定差距原文及已执行的脚本运行记录} / 质量审核{审核/修复规划/修复
执行}),执行链会话在
...
交接文档(subtask auto 子任务或 ondemand 的 `docs/<id>.handoff.md`、handover-test 的
`docs/<id>.testhandoff.md`)时不复用旧会话——其上下文已用满、进度由交接文档承载,
...
- verify:脚本**已执行完毕**(有持久化运行记录)→ 跳过重跑、直接开判定会话
  ...
  修复轮进行中被中断的,凭持久化的判定差距原文重新下发修复提示续跑;
...
隐性阻塞停机);driver 随即开新会话(上下文已超限,会话复用规则自动新建),以续跑
说明(先读交接文档与最近一次输出)继续完成任务,测试仍走同一协议。
...
同现;任务非恢复续跑时,上次尝试遗留的交接文档会被清除(镜像 ondemand 语义)。
