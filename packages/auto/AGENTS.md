...
`@opencode-ai/auto` 是一个私有 CLI(`opencode-auto`)——**专用二次迁移工具(去子命令化,设计基准 docs/specialized-tool-design.md)**:无子命令,直接执行主程序——基于目标目录已有迁移结果(不限于本工具此前轮次)先做一轮前置知识提取(docs/prior-kb/),再自动推进一轮完整 admtvk(分析→设计→迁移实现→测试→验收→知识提炼)二次迁移至结束;中断后再次运行依推导式状态从断点恢复;迁移源/目标缺失时由 AI 依据知识提取结果自动推断并固化。通过 `@opencode-ai/sdk` 的 v2 接口驱动 opencode。注释与用户可见文案使用中文。

## 结构

- `src/index.ts` — CLI 入口(**无子命令**,设计基准 docs/specialized-tool-design.md §2):`opencode-auto [dir] [选项]`,首参数命中旧子命令名(init/continue/run/check/status)即报错指向新用法(退出码 1);--phases/--continue/--commit-subtask 与旧名 --verify-idle/--verify-max(已更名 --idle-time/--idle-max)拦截报文;参数解析(VALUE_FLAGS 带值吞 token,含已移除的 phases/verify-idle/verify-max 以防误当位置参数;BOOLEAN_FLAGS 仅紧随字面量 true/false 时吞值;-p/-m 吞值、-i 布尔;USAGE 为 hoisted 函数 usageText,--help 顶部检查)。**关键参数首跑固化**(-p/--prompt 不入比对、-m/--mode/--agent/--context-limit/--subtask/--verify/--idle-time/--idle-max/--commit/--test-by-driver/--handover-test/--source-dir/--source-path/--dest-dir,parse* 校验同源,非法取值退出码 1;source 两键成对、三迁移键均须为不含 .. 的相对路径,source 首跑显式给出时校验存在性、dest-dir 不校验;handoverTest 须搭配 testByDriver 交叉校验):配置文件缺失 = 首跑,mergeProjectConfig({...CONFIG_DEFAULTS, phases: "admtvk"}, 显式键 + mode)→ saveProjectConfig 固化(mode 优先级 显式值 > legacyModeFallback 旧 .auto/config.json 回落 > 缺省 migrate);**二次运行起逐键冲突校验**——显式给出且与固化值不一致即用法错误(退出码 1,报文含键名与生效值,修订通道 = 直接编辑配置文件),一致视同未给出;二次运行经 loadProjectConfig 装载配置(坏文件退出 1)、loadModes 按配置名解析 mode(未注册退出 1)。运行级参数每次生效不固化(--server/--verbose/--interactive/-i/--wait-answer/--wait-between/--permission/--review/--early/--early-review/--final-review/--dryrun;--review/--early-review 经 parseReviewLimit——缺省 0 不启用、裸选项 3、显式值 1..10 整数,--early 为布尔修饰、review 未启用时单独出现为用法错误,--early-review 是 `--review n --early` 快捷糖、与 --review 同现为用法错误;--final-review 经 parseFinalReviewLimit——缺省 0、裸选项 2、显式值 1..5 整数审计轮上限;--permission 经 parsePermission 缺省/裸选项 ask-deny;--interactive 与 --verbose 互斥检查在此);随后委托 src/tool.ts 的 runTool(config/mode/firstRun/brief/运行级参数)。
- `src/tool.ts` — 主编排 runTool(设计文档 §1 总体流程,逐步幂等):usePromptLibrary 最先装载(校验失败返回 1)→ 模板维护(PLAN.md 缺失/占位态 isPristinePlan → renderPlanScaffold 空模板重建;opencode.json 缺失才创建;.opencode/agent/auto.md 与模板不一致即替换,经 renderText 按 verify/testByDriver 条件渲染)→ -p 整写覆盖 .opencode/auto/brief.md(无 -p 保留既有)→ firstRun 且 verify 未启用打一次 v/verify 正交 note → 打印 formatProjectConfig 配置摘要(testByDriver 启用时打协议横幅)→ .auto/tool.json 完成标记 done → 报告完成退出 0 → manage 启动全程单实例 server → [非 dryrun] 标记不存在且台账覆盖 PHASE_ORDER 全部字母 → archiveRound 归档上一轮(随后 PLAN.md 重置空模板;台账部分完成 = 进行中的本轮,续跑不归档)→ extractPriorKnowledge 前置知识提取(产物 docs/prior-kb/prior-<时间戳>.md,已有非空产物跳过,失败仅 ⚠ 警告)→ 参数推断会话(config.source/destDir 任一缺失时,requireArtifact 骨架,产物协议 .auto/infer.json: 成功 {"sourceDir","sourcePath","destDir"} / {"blocked": 原因};collect 校验合法 JSON + 相对路径 + 源存在性;仅采纳缺失键后 saveProjectConfig 固化;blocked/受阻 → 退出码 2)→ 阶段进度行(formatPhases + currentRound 轮次标注;台账非法仅提示,硬失败在 runAll 预检)→ runAll(phases 恒定 PHASE_ORDER,source/destDir/mode/verify 等透传,managed: server 复用同一实例)→ 退出码 0 且非 dryrun 写完成标记 {done:true}。导出纯函数 parseInferOutput/readToolState 供测试。
- `src/config.ts` — 项目配置层(.opencode/auto/config.json:版本化、随仓库共享、人工可编辑,未知键忽略前向兼容):ProjectConfig 全键(mode/agent/contextLimit/subtask/verify/idleTime/idleMax/commit/testByDriver/handoverTest/phases/source/destDir;看门狗键由 verifyIdle/verifyMax 更名而来,旧键仅在新键缺失时回落读取)与 CONFIG_DEFAULTS(phases 缺省 "m" = 无阶段声明,source/destDir 缺省 undefined,testByDriver/handoverTest 缺省 false);loadProjectConfig 读取校验(文件缺失取缺省并做 legacy 回落——旧 .auto/config.json 的 mode 仅在新文件缺失时生效;坏 JSON/键值越界/mode 未注册/phases 非法/source 形状错误/destDir 非相对路径/handoverTest 未搭配 testByDriver throw 中文报错含键名与期望,CLI 转退出码 1,每次启动经此入口);mergeProjectConfig 仅显式键覆盖(amend 语义);saveProjectConfig 普通整写;formatProjectConfig 一行摘要(启动横幅共用,含"测试 driver on(交接)"段,末尾含阶段);legacyModeFallback 供首跑回落旧位置 mode(配置即唯一事实源,二次运行不再打提示)。
- `src/loop.ts` — 任务循环:取下一个未完成任务执行;启动时 `resetInProgress` 把上次运行中断遗留的 in_progress 重置为 pending,并经 peekProgress 把进度记录处于 verify(--review 启用时)/review 阶段但已被标 done 的任务置回 in_progress(否则 next() 跳过、审核永不补跑);runAll 开头经 usePromptLibrary 装载目标目录提示词覆盖(校验失败返回 1);任务开始横幅;`ensurePointer` 在启动会话前确保 AGENTS.md 指针块、验证/测试执行/提交原则块与维护规则块存在(五个独立标记块,各自幂等补写;验证块随 config.verify、测试块随 config.testByDriver 补写/移除,维护规则块=MAINT_RULE 常量:≤150 行/路由到 docs/agents//更新不追加/只沉淀持久知识,见设计文档 D 节);`ensureGitignore` 确保 tmp/ 与 .auto/ 被 .gitignore 忽略(非 git 目录不动);启动时经 pendingChanges 检测工作区遗留未提交改动并提示会被统一提交纳入;run 前完整性检查(.opencode/agent/<agent>.md 缺失直接报错退出并提示重新运行恢复——tool.ts 模板维护会重建 auto 契约,与模板不一致仅警告);`--dryrun` 权限预检;任务边界统一提交(完成/阻塞/回退 pending 各一次,经 commitTree);verbose 变更文件监视(基于 git status,含子目录中的嵌套 git 仓库);子任务进度上报(每 10 分钟);`--review`/`--early`/verify/`--permission`/idleMs/maxMs/testByDriver/handoverTest/mode/server 句柄透传至 runTask(verify 未启用而 --early 启用时打降级提示);`--final-review` 终审推进挂点(advanceFinal: runTask 完成且任务带 final 标记后路由追加下一任务;next() 为空且终审未完成时打横幅"进入终审闭环"并续跑循环;熔断/报告异常 block 对应终审任务退出码 2);`--interactive` 旁路控制器的创建/回收与 waitBetween 接入;runAll Opts 另加 managed?: ServerHandle——tool.ts 注入时不再自行 manage/close(server 生命周期归调用方,前置会话与主循环共用同一实例),否则照旧自管;阶段化流程(runAll Opts 加 phases/source/destDir,来自配置;tool.ts 恒定传 PHASE_ORDER,`phases === "m"` 走上述原路径零改动):manage 前 routePhase 预检(台账非法等环境错误提前退出 1、不拉 server),主循环抽为 `runTaskLoop(phase)` 闭包(phase 透传 runTask 支撑 v 阶段豁免;finalGate = phase === "m" 才挂 `--final-review` 终审推进,其余阶段忽略并打一次提示),`runPhaseLoop` 按 routePhase 循环推进——`plan` → planPhase(先 snapshotDocs,再组装 handovers——台账中早于当前阶段且已 done 的各阶段归档 handover.md 预拼接,缺文件标注"(无交接文档)",注入纪律只传蒸馏产物不传前序原始 docs/;台账为空时另把 priorKnowledgeDigest(前置知识全文)+ prevRoundDigest(上一轮结论)合并为 prevRound 注入,有轮次归档时打"续轮迁移"提示;以 requireArtifact 骨架开阶段规划会话、产物 = 已填充的 PLAN.md,会话期间 allowWrite(PLAN.md)/finally reprotect,提交 stage=phase-plan)/ `execute` → runTaskLoop / `handover` → handoverPhase(先开蒸馏会话 requireArtifact 产出归档目录 handover.md——四小节协议经 validHandover 校验、reset 清文件重试、受阻返回 2,提交 stage=phase-handover;再 archivePhaseDocs → PLAN.md 拷贝归档后重置空模板 → appendLedger → AGENTS.md >150 行仅 note 不改写 → 统一提交 stage=phase-transition)/ `complete` → 退出 0;plan 路由对 k(知识提炼)阶段走专属分支——不开规划会话、不填 PLAN.md,先 extractKnowledge 知识提取旁路会话(已产出非空文档则幂等跳过;失败仅打 ⚠ 警告不污染退出码、不刷新 docs 快照),随后照常 handoverPhase("k");规划/蒸馏会话受阻均返回 2;交接中断恢复: 归档目录内已有 PLAN.md 而台账缺该字母 → 补台账+提交后续跑(各步幂等)。
...
- `src/phases.ts` — `--phases` 阶段注册表(设计文档 docs/phases-design.md A/C 节,固定六字母不开放自定义):Phase/PHASE_ORDER("admtvk")/phaseText 中文名;parsePhases(非空、字母 ∈ admtvk、不重复、含 m、为 admtvk 子序列——严格递增下标一次遍历,非法返回 null);readLedger 解析 docs/phases.md 台账(推导式状态载体:行协议 `- [done] <letter> <名称> → <归档目录>`,容忍空行与 # 行,字母越界/重复/协议行无法解析 throw 给人工修订指引;文件缺失 = 空台账;行解析抽为 parseLedger/LEDGER_ENTRY 供轮次归档复用)+ appendLedger(查重后追加,重复调用幂等;文件缺失带头部注释创建);PHASE_SLUGS/phaseArchive(归档目录 `docs/phases/<letter>-<slug>/`,a-analysis/d-design/m-migrate/t-testing/v-acceptance/k-knowledge);routePhase(纯路由,由台账+PLAN.md 推导 complete/plan/execute/handover/blocked——blocked = 台账非法或记录了 phases 之外的字母,CLI 转退出码 1);formatPhases(阶段进度行 `a✓ d✓ m▶ t v k`,tool.ts 启动横幅打印);renderPlanScaffold(PLAN.scaffold.md 空模板,verify 条件渲染);snapshotDocs/archivePhaseDocs(阶段开始写 .auto/phase-snapshot.json,交接按"文件名+mtime"差异把 docs/ 变更移入归档目录;排除 docs/phases/、docs/phases.md、docs/agents/,快照缺失退化为移动 A.1 产物目录与 docs/T-*.md);HANDOVER_SECTIONS/validHandover(交接文档四小节协议——关键决策/约束与坑/下一阶段必读清单/产物索引,蒸馏会话产物的 collect 校验用,标题行逐字匹配);续轮机制(M 节,currentRound/archiveRound/prevRoundDigest)——currentRound(当前轮 = docs/phases/ 下 round-<N> 最大编号 + 1,推导式)、archiveRound(tool.ts 启动时归档判定调用(完成标记不存在且台账覆盖全部字母):docs/phases/ 全部阶段归档目录、docs/migration-kb 残留、轮末根 PLAN.md 与台账移入 round-<N>/,台账最后移动作完成标记故中断重跑幂等,并清 .auto/phase-snapshot.json;无可归档内容时不创建目录)、prevRoundDigest(上一轮结论摘录 = 各阶段归档索引 + 最终完成阶段 handover.md 全文 + 迁移知识文档全文,归档内台账用宽松解析坏行不中断;无轮次归档返回 undefined)。
- `src/prompt.ts` — 提示词上下文组装层:文案全部在 `templates/prompts/*.md`(共享片段在 `_partials.md`,经 src/template.ts 渲染),这里把 plan/task/运行信息组装为模板变量;render* 签名稳定(runner/loop/final 调用点不感知模板机制);VERDICT_FILE(`.auto/verify.md`)与 REVIEW_FILE(`.auto/review.md`)判定文件路径、VerifyRun 运行信息类型(含看门狗 timeoutReason);stageText 终审阶段中文名(横幅/loop/final 共用);handoffFile/renderHandoffSteer 交接提示;testHandoffFile(`docs/<id>.testhandoff.md`,与 ondemand handoff 分离命名)与 renderTestResult/renderTestHandover/renderTestContinue 三段测试协议文案(模板 test-result/test-handover/test-continue,steer 反馈与交接后续跑说明;TestRunInfo = VerifyRun+seq);终审四阶段(FinalStage)的职责与报告产出要求内联在 final-task.md 的条件段,renderFinalTask 只传 stage 标志位;renderPhasePlan({phase, brief, handovers, prevRound, source, destDir, mode, verify, finalReview}) 组装阶段规划会话上下文(按 phase 切六阶段职责条件块,brief/handovers(前序交接预拼接)/prevRound(续轮上一轮结论摘录,仅新一轮首个规划会话由 loop 传入)/source.dir/source.path/destDir/mode.init 为输入变量,finalReview 的"终审预留"提醒仅 m 阶段生效——门控在函数内,调用点不必判断);renderPhaseHandover({phase, archive, next, verify}) 组装阶段交接蒸馏会话上下文(archive = phaseArchive(phase),next = 下一阶段"字母 中文名"或 undefined,k 阶段供人工归档措辞);renderKnowledge({file, mode}) 组装 k 阶段知识提取会话上下文(来源=阶段台账与各阶段归档目录,章节骨架/质量约束内联在 knowledge.md,mode.exec 作场景背景注入);renderPriorKnowledge/renderInferSource 组装专用工具的前置知识提取与参数推断会话上下文(模板 prior-knowledge.md/infer-source.md,产物协议 .auto/infer.json)。
- `src/template.ts` — 提示词模板装载与渲染:内置模板经 `with { type: "file" }` 嵌入(embedded 注册表集中登记),readFileSync 在编译产物中同样可读 `/$bunfs` 路径;`usePromptLibrary(dir)` 装载目标目录 `.opencode/auto/prompts/` 同名覆盖(`_partials.md` 按节名合并片段),协议敏感模板(judge/review/verify-script-gen/review-fix/decompose/handoff-steer/final-task/phase-plan/phase-handover/infer-source)覆盖时做关键协议内容校验、缺失即抛错(CLI 转退出码 1);语法 `{{var}}`/`{{#if x}}`/`{{^x}}`/`{{> 片段}}`(块标签独占一行整行吞掉,片段独占一行保留行尾换行并把行首缩进应用到每一行,行内引用仅应用到第二行起);无循环语法——清单类数据由调用方预拼接为字符串。
...
- `src/knowledge.ts` — k(知识提炼)阶段的知识提取编排(phases-design.md P4/D.4,整体认领 fixme-knowledge-design.md 的 --extract-knowledge):knowledgeFile 默认输出路径 docs/migration-kb/migration-<时间戳>.md(时间戳与 run 日志同款)、existingKnowledge 幂等检查(目录内存在非空 .md 即视为已提取)、extractKnowledge(requireArtifact 骨架,伪任务 PLAN,collect 从宽 = 文件存在且非空,提交 stage=knowledge);失败返回 failed 由 loop 打 ⚠ 警告、不污染退出码。**专用工具前置知识提取**(specialized-tool-design.md §3):PRIOR_KB_DIR = docs/prior-kb(与 k 阶段 docs/migration-kb 隔离,archiveRound 不移动)、priorKnowledgeFile(prior-<时间戳>.md)、existingPriorKnowledge(已有非空 .md 即跳过)、extractPriorKnowledge(requireArtifact 骨架,模板 prior-knowledge.md;失败仅警告由 tool.ts 继续)、priorKnowledgeDigest(全文摘录,与 prevRoundDigest 合并注入新一轮首个规划会话)。
...
- `src/protect.ts` — 状态文件只读保护:运行期间 PLAN.md/CURRENT.md/opencode.json
  与 .opencode/auto/config.json 置 0o444(AGENTS.md 不在其列,任务可更新它;人工修订
  配置须在运行外),driver 写入经 allowWrite/reprotect 临时放行,runAll 的 finally 恢复 0o644。
...
- `src/log.ts` — 输出双通道:verbose(文件记录级别)与 foreground(终端明细/时间戳)分离,
  ...
  (interactive 下只进文件);`setInput` 注册交互 readline 后 log 打印先清输入行再重绘;运行时把全部
  输出同步写入目标目录 `.auto/logs/run-<时间戳>.log`(writeSync 逐条直写)。
- `templates/` — 主程序模板维护(src/tool.ts)写入目标目录的模板(`PLAN.scaffold.md`——PLAN.md 空模板,无任务标题行、verify 条件渲染,与交接重置态共用;`opencode.json`、`.opencode/agent/auto.md`——契约含"不得删除或改写任何 opencode-auto 标记块、更新其余内容遵守维护规则块"条款;`PLAN.md` 占位模板现仅 test/prompt.test.ts 防漂移断言引用);`templates/prompts/` 为 20 个会话提示词模板 + `_partials.md` 共享片段(编译期嵌入、运行期渲染,工具不复制,目标目录 `.opencode/auto/prompts/` 同名覆盖);`templates/modes/` 为内置模式文件(目标目录 `.opencode/auto/modes/` 同名覆盖/新增)。
...
- `docs/specialized-tool-design.md` — **去子命令化(专用二次迁移工具)的设计基准,是 CLI 面与总体流程的最新基准**(supersedes init/continue/run/check/status 相关 CLI 表述):无子命令直接执行主程序(前置知识提取 → 参数推断 → 完整 admtvk);四条已确认决策(关键参数二次运行冲突报错退出 1 / check 与 status 彻底删除 / 前置提取失败仅警告 / 完成后重跑报告完成退出 0);总体流程与状态载体(.auto/tool.json 完成标记、docs/prior-kb/、.auto/infer.json 推断协议)、CLI 面(关键参数首跑固化 + 二次逐键冲突校验、运行级参数不固化、历史选项拦截报文)、loop.ts managed server 注入与 prior-kb 注入、删除项与文件级改动清单。**已全部实现(代码与测试落地)。**
...
- **模板必须保持 `with { type: "file" }` 导入**,这是编译时嵌入二进制的唯一方式;
  ...
  会变成 `/$bunfs/...` 导致模板维护失败。分两类登记:主程序模板维护写入目标目录的
  模板在 `src/tool.ts` 顶部导入(opencode.json、.opencode/agent/auto.md;
  PLAN.scaffold.md 在 src/phases.ts 导入);提示词/模式模板分别在
...
- 退出码:`0` 全部完成(台账覆盖 admtvk 全部字母;或 .auto/tool.json 完成标记存在、
  报告"二次迁移已全部完成"短路),`1` 用法/环境错误
  (含旧子命令名/历史选项拦截、关键参数与首跑固化值冲突、agent 契约文件缺失的
  完整性检查、项目配置 .opencode/auto/config.json 非法、
  阶段台账 docs/phases.md 非法或记录了 admtvk 之外的字母),`2` 阻塞或未完成为 pending、等待人工介入
  (阻塞问题写入 PLAN.md;pending 回退不写字段;含参数推断会话受阻或报 blocked、
  阶段规划会话受阻与 --final-review 终审闭环熔断),
...
- 项目配置固化(src/config.ts,设计文档 docs/init-config-agents-design.md 与
  docs/phases-design.md A.2;CLI 面最新基准 docs/specialized-tool-design.md §2):关键参数
  -m/--agent/--context-limit/--subtask/--verify/--idle-time/--idle-max/--commit/--source-dir/--source-path/--dest-dir
  **首次运行时固化**(.opencode/auto/config.json 缺失 = 首跑:显式键 + 缺省值写入,
  phases 恒定 "admtvk";-p/--prompt 整写覆盖 brief.md、不入固化比对;
  source 两键成对给出,首跑显式给出时校验
  <工作目录>/join 后存在(经 stat 跟随软链接——source-dir 可为指向工作目录外的软链,
  断链按不存在拒绝);三迁移键均须为不含 .. 的相对路径,dest-dir 独立固化、
  不校验存在性——迁移目标在 <工作目录>/<dest-dir>,driver 流程文件与迁移产出经它隔离;
  缺失的迁移参数由 tool.ts 的参数推断会话自动推断并写回);**二次运行起逐键冲突校验**——
  显式给出且与固化值不一致即用法错误退出码 1(报文含键名与生效值),一致视同未给出。
  人工修订通道为直接编辑配置文件;坏 JSON/键值越界/mode 未注册时启动即退出
  码 1(严格失败优于静默回落),未知键忽略;旧 .auto/config.json 的 mode 仅在新文件
  缺失时回落读取(首跑生效);运行期间配置文件置只读(brief.md 不在其列,非状态文件);
  启动横幅打印
  formatProjectConfig 一行摘要(含阶段)。判别标准: 改它需同时改 AGENTS.md/PLAN/契约表述或
  描述模型/项目属性 → 关键参数(首跑固化);只描述本次运行怎么跑、人怎么盯 → 运行参数(每次生效)。
- 模板维护与 brief(tool.ts,每次运行幂等):PLAN.md 缺失/占位态 → 空模板
  (templates/PLAN.scaffold.md)重建,交给阶段规划会话;opencode.json 缺失才创建;
  .opencode/agent/auto.md 与模板不一致即替换;`-p/--prompt` 整写覆盖
  .opencode/auto/brief.md(项目意图,版本化、人工可编辑,每次运行均可重写,无 -p 保留
  既有),由前置知识提取、参数推断与阶段规划会话消费;
  首跑且 verify 未启用时打一次 v/verify 正交 note(流程恒含 v 阶段)。
- 阶段循环(流程恒定 admtvk,tool.ts 恒传 PHASE_ORDER;P1..P4 已接线;设计文档 phases-design.md D/E/F 节):阶段
  状态是推导式的,routePhase 只读 docs/phases.md 台账与 PLAN.md(零新增持久化状态),
  主程序据此循环——PLAN.md 为空模板 → 开阶段规划会话(旁路一次性,复用 requireArtifact
  ...
  admtvk 全部字母 → 退出 0(tool.ts 写 .auto/tool.json 完成标记)。`--final-review` 只在 m 阶段挂接(其余阶段打一次
