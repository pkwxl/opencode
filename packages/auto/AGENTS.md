...
- `-m/--mode` 模式层:提示词级场景引导,不影响 driver 调度状态机——ModeSpec 三段
  ...
  一个协议完整的 .md 文件,零源码改动。init 与 run 均接受,解析优先级 显式值 >
  持久化值(.auto/config.json 的 mode,resolveModeFlag 处理)> migrate,成功后回写
  持久化;未注册名为用法错误退出码 1(报文列出当前支持的模式),显式值与持久化值
  不一致时警告。
