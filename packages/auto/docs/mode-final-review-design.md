...
| 终审阶段载体 | 真任务入 PLAN.md(`T-F<k>` ID + `final: <stage>@<round>` 字段标记),复用 runTask 全流水线(分解/子任务会话/收尾/三段式验收/CURRENT.md/进度恢复/commit 档位),driver 只做"生成任务 → 跑任务 → 解析报告路由"的状态机 |
| audit/validate 任务的逐任务审核 | 跳过(它们本身就是审核;审核套娃浪费会话且语义混乱):runTask 内按 final 标记强制 review=0;refactor/patch/finalize 任务在 `--review` 启用时正常审核 |
...
| `--subtask off/ondemand` | 终审任务遵循全局档位;off 下终审任务 verify 差距 → 回退 pending、退出码 2(既有语义) |
...
[原任务全部 done](next() 返回空,终审未完成)
 ...
 → driver 解析并 appendTask:T-F1,字段 final: audit@1,verify = 结构检查命令
 → 主循环 next() 拾取 → runTask(T-F1) 全流水线(分解/子任务/收尾/三段式验收)
 → runTask 完成且任务带 final 标记 → 路由:解析 docs/final/audit-r1.md 末行
     策略: 无        → 生成 Finalize 任务 → 执行 → commit once(既有位置)→ 退出 0
     策略: 重构|修补  → 生成 remediate 任务(verify 取自提案)→ 执行
                      → 生成 validate 任务(verify = 结构检查)→ 执行
...
verify: command: <命令>          ← 可选;refactor/patch 建议给出(通常复用原任务的
                                   验证命令);audit/validate/finalize 由 driver 固定,
                                   提案中的该行被忽略
```
...
driver 解析提案后 `appendTask`:ID/`final` 字段/audit-validate 的固定 verify 由
driver 决定,标题与正文取自提案,verify 字段取提案的 `verify:` 行(refactor/patch;
缺省回落自然语言 → generate 分支)。

### B.4 报告协议与三段式验收的配合

- **audit 任务**:正文要求产出审计报告 `docs/final/audit-r<N>.md`,末两行
  `结论: <概述>`、`策略: 重构|修补|无`;driver 固定 verify 为结构检查,如
  `command: test -s docs/final/audit-r1.md && grep -qE '^策略[:：] ?(重构|修补|无)$' docs/final/audit-r1.md`。
- **validate 任务**:报告 `docs/final/validate-r<N>.md`,末行
  `结论: 通过` 或 `结论: 差距 <描述>`;verify 同为结构检查。
- **协议自愈**:策略/结论行缺失或取值非法 → 结构检查失败 → 三段式验收判差距 →
  既有 FIX_ROUNDS 修复循环反馈回执行会话,无需专门的协议重试逻辑。
- **remediate 任务**:报告 `docs/final/refactor-r<N>.md` / `patch-r<N>.md`,
  自由正文无协议;verify 取自提案命令。
- **finalize 任务**:收尾报告 `docs/final/finalize.md`(自由正文);verify 缺省为
  结构检查,提案可覆盖。
...
- audit/validate 终审任务强制 `review = 0`(任务对象带 `final` 字段且 stage ∈
  {audit, validate} 时;`--early` 随之自然失效);
...
4. 终审任务 done 但报告缺失/协议非法 → 不可能(结构检查验收已拦截);报告被人工
   删除 → 重新路由时按"任务存在但无报告"处理为阻塞,提示人工核查;
...
| `src/runner.ts` | Opts.mode;audit/validate 强制 review=0;导出 requireArtifact/runSession | P1 / P2 |
...
- **提案 verify 命令的注入信任**:refactor/patch 任务的 verify 命令来自生成会话
  提案,提示词须约束"优先复用原任务的验证命令/测试套件,不得发明未运行过的检查";
  三段式判定会话对不合理脚本有重验护栏,风险可控。
