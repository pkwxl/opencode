...
2. 阅读 PLAN.md 模板,理解其格式(任务标题 `## T-NNN: 标题 [pending]`、紧跟标题的
   字段行{{#if verify}},如验收标准字段 `  - verify: command: <命令>`{{/if}});
3. 根据下方需求,把 PLAN.md 填充为一份可执行的实施计划:任务按依赖顺序排列,不要手工
   编写子任务检查项(driver 会自动分解);{{#if verify}}每个任务带 verify 验收标准
   (具体命令用 `command: ` 前缀,或自然语言描述);
...
5. {{/if}}如执行计划需要访问项目目录外的路径或特殊命令,在 opencode.json 的 permission
   规则中补充放行。
