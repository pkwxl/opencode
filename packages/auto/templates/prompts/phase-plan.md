...
以下是各前序阶段的交接蒸馏文档,是跨阶段记忆的唯一通道(代替前序原始 docs/,
规划时不要试图读取它们;需要更多细节时按其中的产物索引自行取用):
...
的工作。上一轮结论摘录如下(完整产物在轮次归档目录内,需要细节时按索引取用):
...
{{#if phaseA}}- 摸清源系统与源模块的外部行为、依赖与边界,为后续阶段提供行为基线;产物写入
  docs/analysis/(行为基线、依赖清单)。
- 本阶段是首个阶段: 把对源系统的勘察计划排为首批任务。
{{/if}}
{{#if phaseD}}- 完成目标系统侧的模块设计(接口、数据结构、适配点);产物写入 docs/design/。
{{/if}}
{{#if phaseM}}- 完成代码迁移与改造;产物为源码改动与 docs/ 下的任务报告。
{{/if}}
{{#if phaseT}}- 完成测试体系迁移/补齐,对基线行为做回归覆盖;产物为测试代码与 docs/testing/。
{{/if}}
{{#if phaseV}}- 对照基线与需求完成整体验收;产物写入 docs/acceptance/(验收报告)。
{{/if}}
{{#if phaseK}}- 完成迁移知识沉淀;产物写入 docs/migration-kb/。
{{/if}}
## 任务
...
3. 任务编号自 T-001 连续递增;每个任务聚焦一个可独立交付的成果;不要手工编写子任务
