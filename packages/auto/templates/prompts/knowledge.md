...
- 阶段台账 docs/phases.md: 各已完成阶段的归档目录索引;
- 各阶段归档目录 docs/phases/<字母>-<名称>/: 优先细读其中的 handover.md(该阶段
  的蒸馏结论),需要更多细节时按其「产物索引」小节取用归档的原始产物(含各阶段
  归档 PLAN.md 的任务、验收与阻塞问答轨迹);
...
1. 只读勘察: 读 docs/phases.md 与各归档目录的 handover.md,把握迁移全貌;需要
   细节时按产物索引取用归档产物,不要跳过尚未读过的阶段;
