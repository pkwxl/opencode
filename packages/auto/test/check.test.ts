...
describe("checkPrinciple", () => {
  test("标记要求会话亲自运行验证的描述,放行合规语句", async () => {
    ...
    try {
      ...
      // verify 字段行、否定句、driver 归属句均不计;缺验证原则块给出提示
      expect(notes).toEqual([`AGENTS.md 缺少验证原则块,运行 opencode-auto init 可补写`])
    } finally {
  ...
  test("AGENTS.md 的 opencode-auto 块整体跳过;init 补写后无提示", async () => {
    ...
    try {
      ...
      // init 补写指针块与验证原则块后,原则块内的 driver 执行表述不再触发提示
      const ensured = await ensurePointer(dir)
      expect(ensured).toEqual({ pointer: true, principle: true })
