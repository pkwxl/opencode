...
## T-002: 选择问候语并写入 greeting.txt [pending]
  - verify: test -f greeting.txt
这个任务缺少关键信息:你不知道问候语应该用中文还是英文。
调用 question 工具询问应该使用哪种语言,拿到答案后把问候语写入 greeting.txt。
...
test.skipIf(!E2E)(
  ...
  async () => {
    ...
    try {
      ...
      // 模拟人工介入: 填写 answer 后重启
      const text = (await Bun.file(join(dir, "PLAN.md")).text()).replace(
        /(## T-002:[^\n]*\n(?:  - [^\n]*\n)*)/,
        '$1  - answer: "使用中文"\n',
      )
      await Bun.write(join(dir, "PLAN.md"), text)

      // 第二轮: T-002 携问答历史续跑,T-003 完成,全部 done
