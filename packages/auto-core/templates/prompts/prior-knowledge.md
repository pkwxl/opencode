...
- docs/ 全树: 已有迁移的文档产物;交接文档(docs/handovers/)、迁移知识
  (docs/migration-kb/,R<N>- 前缀按轮次)与此前轮次的前置知识(docs/prior-kb/,
  同样 R<N>- 前缀)是此前蒸馏的结论,优先细读;docs/phases/ 下的阶段/轮次归档
  只含过期状态(阶段 PLAN 快照),需要细节时按「产物索引」小节取用;
