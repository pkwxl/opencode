...
- docs/ 全树: 已有迁移的文档产物;docs/phases/ 下若存在 <字母>-<名称>/ 阶段归档
  或 round-<N>/ 轮次归档,优先细读其中的 handover.md 与 migration-kb/ 知识文档
  (此前蒸馏的结论)与 prior-kb/ 前置知识归档(此前轮次的蒸馏结论,同样重新蒸馏
  而非照搬),需要更多细节时按其「产物索引」小节取用归档的原始产物;
