...
- 瞬时会话错误的重试阶梯与人工裁决(OPENCODE_AUTO_RETRY_WAITS / _RETRY_ASK,
  session-error-retry-plan.md「2026-09-12 修正二」):阶梯由一张分钟表描述,每个元素
  是该次重试前的等待、元素个数即重试上限,**缺省 `0,1,2,4,8`** = 五次重试,首次立即
  (瞬时抖动常在下一回合就恢复),其后 1/2/4/8 分钟。退避从分钟起步而非秒级翻倍:
  opencode 内层每次故障已用掉 6×300s 超时 + 60s 退避 ≈ 31 分钟,外层再叠秒级曲线只
  占零头;分钟级等待的意义只在跨过一段上游退化。`off` = 不自动重试,首次失败即进
  人工裁决。阶梯耗尽后**先等人工再决定去留**(`OPENCODE_AUTO_RETRY_ASK`,缺省 30
  分钟;0 = 不等、直接阻塞):答 continue/继续 则阶梯从头再走一轮,答 exit/退出 立即
  阻塞,超时或答非所问按回落处理(本分支即阻塞)。等人工而不是直接退出,是因为优雅
  阻塞退出写的是 active=false 的恢复点、重跑明确不复用旧会话(resume.ts 的既定语义:
  人工介入可能耗时数小时且会改动环境),而超时故障里人什么都没改、只是等——留在进程
  里等,上一条刚保住的会话才还活着、还能继续分叉。无人值守的跑批不会因此卡死:无
  应答一律回落。
